...
type Article = { source: string; title: string; href: string };

export default function EmpresaPage() {
  const { raw, t } = useLanguage();
  const metrics = raw<Metric[]>('site.company.metrics');
  const articles = raw<Article[]>('site.company.articles');
  ...
  return (
    <div className="flex flex-col">
      ...
      <section className="py-20 sm:py-28 lg:py-36 bg-white">
        <div className="layout-shell">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-neutral-400 mb-10">
            {t('site.company.originEyebrow')}
          </p>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24">
            <h2 className="text-4xl sm:text-5xl lg:text-6xl font-bold leading-[1.05] [font-family:var(--font-display-family)]">
              {t('site.company.originTitle')}
            </h2>
            <div className="flex flex-col justify-start gap-8">
              <p className="text-xl text-neutral-600 leading-relaxed">
                {t('site.company.originCopy1')}
              </p>
              <p className="text-xl font-semibold text-neutral-900 leading-relaxed">
                {t('site.company.originCopy2')}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="layout-shell">
            <div className="flex flex-col items-center text-center gap-10">
              ...
              <Button asChild size="lg" className="min-w-48">
                <Link href="/casos-de-uso">{t('site.company.storiesCta')}</Link>
              </Button>
            </div>
          </div>
        </FeaturePanel>
            ...
            {articles.map((article) => (
              ...
              >
                ...
                <div className="flex items-center">
                  <ExploreButton
                    href={article.href}
                    title={t('site.company.readArticle')}
                    color="black"
                  />
                </div>
              </a>
